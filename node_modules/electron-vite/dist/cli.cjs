...
cli
    ...
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/lib-1z8RxDdf.cjs'); });
...
cli.command('build [root]', 'build for production').action(async (root, options) => {
    const { build } = await Promise.resolve().then(function () { return require('./chunks/lib-ikhSwZ8P.cjs'); });
    const inlineConfig = createInlineConfig(root, options);
    if (options.entry) {
        process.env.ELECTRON_ENTRY = options.entry;
    }
...
cli
    ...
    .action(async (root, options) => {
    const { preview } = await Promise.resolve().then(function () { return require('./chunks/lib-Nm93Me1W.cjs'); });
    const inlineConfig = createInlineConfig(root, options);
    if (options.noSandbox) {
        process.env.NO_SANDBOX = '1';
    }
